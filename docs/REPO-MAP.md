...
| `_verify_cache.py` | Cache of proven signatures for the recall hot path (~0.4–0.8 s saved per recall) | `load_for_store` (stat **before** read), `VerifiedSignatureCache` — HMAC-keyed digests, forgery needs key-file read access |
...
- `_verify_cache`: only positive verifications are cached; a hit skips only
  ...
  check) runs on every recall, warm or cold.
